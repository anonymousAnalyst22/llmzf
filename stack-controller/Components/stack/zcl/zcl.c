...
zclProcMsgStatus_t zcl_ProcessMessageMSG( afIncomingMSGPacket_t *pkt )
{
  ...
  {
    /**
     * JC NOTES: Here we remove the check of manuSpecific field for ZCL foundation command.
     * The reason is that we found for some vendors, e.g., Philips, they (maybe accidentally) set this field, and as a result, foundation command (e.g., Default Response) cannot be further proceessed.
    */
