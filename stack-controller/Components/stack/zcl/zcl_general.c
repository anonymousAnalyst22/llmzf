...
#endif // ZCL_IDENTIFY
static ZStatus_t zclGeneral_ProcessInIdentityJC( zclIncoming_t *pInMsg, zclGeneral_AppCallbacks_t *pCBs );
...
static ZStatus_t zclGeneral_HdlInSpecificCommands( zclIncoming_t *pInMsg )
{
  ...
  switch ( pInMsg->msg->clusterId )
  {
    ...
    case ZCL_CLUSTER_ID_GEN_IDENTIFY:
      ...
      // JC NOTE: Add support for handling ZCL Identify Query Response
      stat = zclGeneral_ProcessInIdentityJC(pInMsg, pCBs);
      break;
...
/*********************************************************************
 * @fn      zclGeneral_ProcessInIdentityJC
 *
 * @brief   Process in the received Identity Query Response Command.
 *
 * @param   pInMsg - pointer to the incoming message
 *
 * @return  ZStatus_t
 */
static ZStatus_t zclGeneral_ProcessInIdentityJC( zclIncoming_t *pInMsg,
                                               zclGeneral_AppCallbacks_t *pCBs )
{
  if ( zcl_ServerCmd( pInMsg->hdr.fc.direction ) )
  {
    // We do not care about the ZCL Identify command sent to the coordinator
  }
  else // Client Command
  {
                                                   ...
                                                   zclGeneral_AppCallbacks_t *pCBs )
{
  ...
  switch ( pInMsg->hdr.commandID )
  {
    // JC NOTE: Extend the original z-stack to let it parse enhanced view/add commands
    case COMMAND_SCENE_ENHANCED_VIEW_RSP:
    ...
    // JC NOTE: Extend the original z-stack to let it parse enhanced view/add commands
    case COMMAND_SCENE_ENHANCED_ADD_RSP:
    case COMMAND_SCENE_REMOVE_RSP:
    case COMMAND_SCENE_REMOVE_ALL_RSP:
    case COMMAND_SCENE_STORE_RSP:
    // JC NOTE: Extend the original z-stack to let it parse Copy Scene Response command.
    // The intuition is that the payload for the command is the same as COMMAND_SCNE_ADD_RSP
    case COMMAND_SCENE_COPY_RSP:
      if ( pCBs->pfnSceneRsp )
      {
