                     ...
                     uint32 timestamp, uint8 radius )
{
  ...
  while ( epDesc )
  {
    ...
    /**
     * JC NOTES: Here we temporally remove the first check, as some devices (e.g., Philips Hue) do not strictly follow the rule that "an endpoint can only support one profile".
    */

    //if ( (aff->ProfileID == epProfileID) ||
    //     ((epDesc->endPoint == ZDO_EP) && (aff->ProfileID == ZDO_PROFILE_ID)) ||
    //     ((epDesc->endPoint != ZDO_EP) && ( aff->ProfileID == ZDO_WILDCARD_PROFILE_ID )) )
    if ((epDesc->endPoint != ZDO_EP) && (aff->ProfileID >=0) ||
      ((epDesc->endPoint == ZDO_EP) && (aff->ProfileID == ZDO_PROFILE_ID)) ||
         ((epDesc->endPoint != ZDO_EP) && ( aff->ProfileID == ZDO_WILDCARD_PROFILE_ID )) )
    {
